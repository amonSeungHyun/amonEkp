...
<script type="text/javascript">
	...
	function goModify() {
         ...
         frm.method = "POST";
         frm.action = "/workflow/modify.yolo";
         frm.submit();
	...
	function goReadDocument(doc_no,emp_no) {
	  ...
	  $.ajax({
		  url:"/workflow/documentDetail.yolo",
		  data:{"doc_no":doc_no,
			  	"emp_no":emp_no},
		  dataType:"JSON",
		  success:function(json){
			    	...
			    	"<div style='padding : 10px;  padding-bottom: 40px; margin-top: 20px;'  class='border-bottom'>"+
						...
						"<div class='collapse' id='collapseExample'>"+
						 ...
						 if(json.orgfilename != null){
						  html+= "<a href='/download.yolo?doc_no="+json.doc_no+"'>"+json.orgfilename+"</a>"+
						  "<br><img src='/files/workflow/"+json.filename+"' style='width=100px; height=100px;'>";
						 }		  
						 else {
							 html+="첨부파일이 없습니다.";
						 }
	...
	function approvalModal(doc_no, emp_no) {
		  ...
		  $.ajax({
			  url:"/workflow/modalApproval.yolo",
			  data:{"doc_no":doc_no,
				  	"emp_no":emp_no},
			  dataType:"JSON",
			  success:function(json){
  ...
  function myDocument(currentShowPageNo) {
	  ...
	  $.ajax({
		  url:"/workflow/myDocument_dm.yolo",
