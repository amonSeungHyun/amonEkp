			...
			//	console.log(json);
				...
				if(json.fileName != null){
					...
					console.log(json.fileName);
					 ...
					 $.ajax({
					    	url : "<%= request.getContextPath() %>/notice/getMyFile.yolo",
								...
								if(json2.length>0){
									...
									$.each(json2, function(index2, item2){
										html+='<span style="font-size: 10pt; color: gray;"><i class="fas fa-solid fa-paperclip ml-3 mr-1"></i></span>'+
								        	'<span id="notifile" style="cursor:pointer;" onclick="javascript:location.href=\'<%= request.getContextPath() %>/notice/downloadMyFile.yolo?fileName='+item2.fileName+'&orgFilename='+item2.orgFilename+'\'" >'+item2.orgFilename+'</span><br>';
									});
